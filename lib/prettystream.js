...
function PrettyStream(opts){
  ...
  function extractCustomDetails(rec){
    ...
    Object.keys(rec).forEach(function(key) {
      if (skip.indexOf(key) === -1){
        ...
        if (value.indexOf('\n') !== -1 || (value.length > 50 && !config.singleLine)) {
  ...
  this.formatRecord = function formatRecord(rec){
    ...
    details = stylize((details.length && !config.singleLine ? details.join('\n    --\n') + '\n' : ''), 'grey');
    ...
    if (config.mode === 'short'){
      var formatString = '[%s] %s %s'; // level already includes a space
      var formatArguments = [time, level, name];

      if (src) {
        formatString += ' %s';
        formatArguments.push(src);
      }

      if (msg) {
        formatString += ' %s';
        formatArguments.push(msg);
      }

      if (extras || details) {
        formatString += '%s\n%s';
        formatArguments = formatArguments.concat([extras, details]);
      } else {
        formatString += '\n';
      }

      formatArguments.unshift(formatString);
      return format.apply(this, formatArguments);
    }
    if (config.mode === 'dev'){
